...
pub fn qrcode_matrix(message: &str) -> Vec<Vec<bool>> {
    let mut qrcode_matrix = qrcode_generator::to_matrix(message, QrCodeEcc::High)
        .expect("Unable to create QR Code for this message");

    let draw_emoji = (9. * 9. / (qrcode_matrix.len() * qrcode_matrix[0].len()) as f32) < 0.15;
    ...
    for line_id in 0..qrcode_matrix.len() {
        for block_id in 0..qrcode_matrix[line_id].len() {
            if (line_id < 7 && block_id < 7)
                ...
                || (draw_emoji
                    && line_id <= (qrcode_matrix.len() / 2) + 4
                    && line_id >= (qrcode_matrix.len() / 2) - 4
                    && block_id <= (qrcode_matrix[line_id].len() / 2) + 4
                    && block_id >= (qrcode_matrix[line_id].len() / 2) - 4)
            {
...
pub fn qrcode_generate(message: &str) -> ImageBuff8 {
    ...
    let mut qrcode_image = ImageBuff8::from_fn(
        qrcode_matrix[0].len() as u32 * 35,
        qrcode_matrix.len() as u32 * 35,
        |_x: u32, _y: u32| Rgb {
            data: [255, 255, 255],
        },
    );
        ...
        for block_id in 0..qrcode_matrix[line_id].len() {
            if qrcode_matrix[line_id][block_id] {
                let bits = [
                    ...
                    block_id != qrcode_matrix[line_id].len() - 1
                        && qrcode_matrix[line_id][block_id + 1],
                ...
                copy_image_into_another(
                    &mut qrcode_image,
                    &get_block(bits, &block_table).to_rgb(),
                    block_id as u32 * 35,
                    line_id as u32 * 35,
                );
            }
    ...
    copy_image_into_another(
        &mut qrcode_image,
        &eye.rotate270().to_rgb(),
        0,
        (qrcode_matrix.len() as u32 - 7) * 35,
    );
    copy_image_into_another(
        &mut qrcode_image,
        &eye.rotate90().to_rgb(),
        (qrcode_matrix[0].len() as u32 - 7) * 35,
        0,
    );

    if (9. * 9. / (qrcode_matrix.len() * qrcode_matrix[0].len()) as f32) < 0.15 {
        copy_image_into_another(
            &mut qrcode_image,
            &image_center.to_rgb(),
            (qrcode_matrix.len() as u32 / 2 - 4) * 35,
            (qrcode_matrix[0].len() as u32 / 2 - 4) * 35,
        );
    }
