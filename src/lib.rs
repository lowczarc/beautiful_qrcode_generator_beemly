...
pub fn qrcode_generate(message: &str) -> ImageBuff8 {
    ...
    let eye = open(Path::new("assets/eye.png")).unwrap();
    let image_center = open(Path::new("assets/image_center.png")).unwrap();
    let block_table: [DynamicImage; 4] = [
        open(Path::new("assets/0.png")).unwrap(),
        open(Path::new("assets/1.png")).unwrap(),
        open(Path::new("assets/2.png")).unwrap(),
        open(Path::new("assets/3.png")).unwrap(),
    ];
